...
+ Move left: A/Left
+ Move right: D/Right
+ [Soft Drop](https://tetris.wiki/Drop#Soft_drop): W/Down
+ [Sonic Drop](https://tetris.wiki/Drop#Instant_drop): Space/Up
+ Rotate clockwise: K/X
+ Rotate counterclockwise: J/Z

Each iteration of the game loop executes one _frame_ of the game and the loop runs at a fixed rate (60 frames per second). In the loop, we call the paintImmediately method of the game engine to advance the game state by one frame. We then trigger a re-rendering of the game to the screen via the `paintImmediately` method.

## Game Engine

The Engine class represents the core game engine. A blocky game consists of two components, the well and the currently active piece, along with auxiliary state. When running a frame, the following methods are called:
...
The Piece class captures the active piece. A piece is a [tetromino](https://en.wikipedia.org/wiki/Tetromino). The different kinds of pieces can be found in the PieceKind.java enum and the layouts of each piece can be found in the data directory of the project.
...
The BlockyPanel class is responsible for rendering the game to the screen. The `paintComponent` method performs the actual drawing by, first, rendering the active piece and, then, the well grid.